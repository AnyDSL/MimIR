...
TEST(Lexer, Loc) {
    ...
    // clang-format off
    EXPECT_EQ(t1.loc(), Loc(sym, {1,  2}, {1,  5}));
    EXPECT_EQ(t2.loc(), Loc(sym, {1,  8}, {1, 10}));
    EXPECT_EQ(t3.loc(), Loc(sym, {1, 15}, {1, 17}));
    EXPECT_EQ(t4.loc(), Loc(sym, {1, 19}, {1, 20}));
    EXPECT_EQ(t5.loc(), Loc(sym, {2,  1}, {2,  5}));
    EXPECT_EQ(t6.loc(), Loc(sym, {2,  7}, {2,  7}));
    EXPECT_EQ(t7.loc(), Loc(sym, {2,  9}, {2, 11}));
    EXPECT_EQ(t8.loc(), Loc(sym, {2, 14}, {2, 14}));
    // clang-format on
}
...
TEST_P(Real, sign) {
    ...
    Lexer l1(w, w.sym("<is1>"), is1);
    EXPECT_ANY_THROW(l1.lex());

    std::istringstream is2("2.34e");
    Lexer l2(w, w.sym("<is2>"), is2);
    EXPECT_ANY_THROW(l2.lex());
}
