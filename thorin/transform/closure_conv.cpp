...
void ClosureConv::run() {
    ...
    while (!worklist_.empty()) {
        ...
        if (auto i = closures_.find(def); i != closures_.end()) {
            ...
            auto params = world().tuple(DefArray(old_fn->num_doms(), [&](auto i) { return new_fn->var(i + 1); }),
                                        world().dbg("cc_param"));
            subst.emplace(old_fn->var(), params);

            auto filter = (new_fn->filter()) ? rewrite(new_fn->filter(), subst) : nullptr; // extern function

            auto body = (new_fn->body()) ? rewrite(new_fn->body(), subst) : nullptr;
...
const Def* ClosureConv::rewrite(const Def* def, Def2Def& subst) {
    ...
    if (auto nom = def->isa_nom()) {
        ...
        if (auto app = def->isa<App>(); app && new_ops[0]->type()->isa<Sigma>()) {
            ...
            return map(world().app(
                fn, DefArray(num_doms(fn), [&](auto i) { return (i == 0) ? env : world().extract(args, i - 1); })));
        } else {
...
const Def* ClosureConv::closure_type(const Pi* pi, Def2Def& subst, const Def* env_type) {
    if (!env_type) {
        ...
        auto dom = world().sigma(
            DefArray(pi->num_doms() + 1, [&](auto i) { return (i == 0) ? env_type : rewrite(pi->dom(i - 1), subst); }));
...
void FVA::split_fv(const Def* def, DefSet& out) {
    if (def->no_dep() || def->isa<Lam>() || def->is_external() || def->isa<Axiom>()) {
        ...
        for (auto op : tuple->ops()) split_fv(op, out);
    } else {
        out.emplace(def);
    }
}
...
std::pair<FVA::Node*, bool> FVA::build_node(Lam* lam, NodeQueue& worklist) {
    ...
    node->fvs     = DefSet();
    for (auto v : scope.free_defs()) { split_fv(v, node->fvs); }
    node->preds    = Nodes();
    node->succs    = Nodes();
...
ClosureConv::Closure ClosureConv::make_closure(Lam* fn, Def2Def& subst) {
    if (auto i = closures_.find(fn); i != closures_.end()) return i->second;
