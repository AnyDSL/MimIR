...
Lam* Clos2SJLJ::get_throw(const Def* dom) {
    ...
    if (inserted || !tlam) {
        ...
        auto m2                = w.call<mem::store>({m1, r, var});
        rbuf                   = core::op_bitcast(mem::type_ptr(mem::type_ptr(var->type())), rbuf);
        auto m3                = w.call<mem::store>({m2, rbuf, r});
...
Lam* Clos2SJLJ::get_lpad(Lam* lam, const Def* rb) {
    ...
    if (inserted || !lpad) {
        ...
        auto [m1, arg_ptr]      = w.call<mem::load>({m, rb})->projs<2>();
        arg_ptr                 = core::op_bitcast(mem::type_ptr(dom), arg_ptr);
        auto [m2, args]         = w.call<mem::load>({m1, arg_ptr})->projs<2>();
