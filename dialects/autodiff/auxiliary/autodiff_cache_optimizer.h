...
public:
    ...
    bool requires_cache(const Def* def) {
        if (is_load_val(def) && war_analysis.is_overwritten(def)) { return true; }
    ...
    bool needs_cache(const Def* def) {
        if (auto load = is_load_val(def)) {
            if (!war_analysis.is_overwritten(def)) { return false; }
    ...
    DefSet optimize(DefSet defs) {
        DefSet filtered;
        for (auto def : defs) {
            if (needs_cache(def) && !depends_on_cache(def, defs)) { filtered.insert(def); }
        }
