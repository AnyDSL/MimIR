...
const Def* Reshape::convert(const Def* def) {
    ...
    if (!pi_ty) return def;
    auto target_ty = target_types_[def];
    auto new_ty    = target_ty ? target_ty : convert_ty(pi_ty);
...
const Def* Reshape::rewrite_convert(const Def* def) {
    ...
    if (def->isa<Var>() || def->isa<Axiom>() || def->isa<Global>()) { return def; }

    for (auto& plugin : plugins_) {
        auto result = plugin.rewrite(*this, def);
    ...
    const Def* new_type;
    if (def->isa_nom<Lam>()) {
        new_type = convert_ty(def->type());
    } else {
        new_type = rewrite(def->type());
    }
    ...
    if (auto malloc = match<mem::malloc>(def)) {
        ...
        if (callee->isa_nom<Lam>()) { arg = reshape(arg, callee->type()->as<Pi>()); }
