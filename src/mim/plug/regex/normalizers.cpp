...
template<quant id> const Def* normalize_quant(const Def* type, const Def* callee, const Def* arg) {
    ...
    if (mim::isa(id, arg)) return arg;

    if constexpr (id == quant::plus) {
        // (\d?)+ and (\d*)+ == \d*
        if (auto optional_app = mim::isa(quant::optional, arg))
            return world.call(quant::star, optional_app->arg());
        else if (auto star_app = mim::isa(quant::star, arg))
            return arg;
    } else if constexpr (id == quant::star) {
        // (\d?)* and (\d+)* == \d*
        if (auto quant_app = mim::isa<quant>(arg)) return world.app(callee, quant_app->arg());
    } else if constexpr (id == quant::optional) {
        // (\d*)? and (\d+)? == \d*
        if (auto star_app = mim::isa(quant::star, arg))
            return arg;
        else if (auto plus_app = mim::isa(quant::plus, arg))
...
template<class ConjOrDisj> void flatten_in_arg(const Def* arg, DefVec& new_args) {
    for (const auto* proj : arg->projs()) {
        ...
        if (auto seq_app = mim::isa<ConjOrDisj>(proj))
...
bool compare_re(const Def* lhs, const Def* rhs) {
    auto lhs_range = mim::isa<range>(lhs);
    auto rhs_range = mim::isa<range>(rhs);
...
auto get_range(const Def* rng) -> Range {
    auto rng_match = mim::isa<range, false>(rng);
...
void merge_ranges(DefVec& args) {
    ...
    while (ranges_begin != args.end() && !mim::isa<range>(*ranges_begin)) ranges_begin++;
...
template<cls A, cls B> bool equals_any(const Def* cls0, const Def* cls1) {
    return (mim::isa(A, cls0) && mim::isa(B, cls1)) || (mim::isa(A, cls1) && mim::isa(B, cls0));
}

bool equals_any(const Def* lhs, const Def* rhs) {
    auto check_arg_equiv = [](const Def* lhs, const Def* rhs) {
        if (auto rng_lhs = mim::isa<range>(lhs))
            if (auto not_rhs = mim::isa<not_>(rhs)) {
                if (auto rng_rhs = mim::isa<range>(not_rhs->arg())) return rng_lhs == rng_rhs;
            }
        return false;
    };
...
bool equals_any(Defs lhs, Defs rhs) {
    ...
    auto only_ranges = std::ranges::views::filter([](auto d) { return isa<range>(d); });
...
const Def* normalize_disj(const Def* type, const Def*, const Def* arg) {
    ...
    if (arg->as_lit_arity() > 1) {
        auto contains_any = [](auto args) {
            return std::ranges::find_if(args, [](const Def* ax) -> bool { return mim::isa<any>(ax); }) != args.end();
        };
        ...
        for (const auto* cls0 : new_args) {
            ...
            if (auto not_rhs = mim::isa<not_>(cls0)) {
                if (auto disj_rhs = mim::isa<disj>(not_rhs->arg())) {
