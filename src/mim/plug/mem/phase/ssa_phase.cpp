...
const App* get_slot(const Def* ptr) {
    if (auto extract = ptr->isa<Extract>()) {
        return Axm::isa<mem::slot>(extract->tuple());
    }
    return nullptr;
}
...
const Def* SSAPhase::concr2abstr_impl(const Def* def) {
    ...
    if (auto branch = Branch(def)) {
        ...
        if (auto slot = get_slot(ptr)) {
            ...
            if (auto [_, value] = mem_load(map, id); value) return old_world().tuple({map, value});
        }
