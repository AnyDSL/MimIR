...
const Def* LowerFor::rewrite_imm_App(const App* app) {
    if (auto for_ax = Axm::isa<affine::For>(app)) {
        ...
        auto [old_begin, old_end, old_step, old_init, old_body, old_exit] = for_ax->args<6>();
        ...
        auto new_begin = rewrite(old_begin);
        auto new_end   = rewrite(old_end);
        auto new_step  = rewrite(old_step);
        auto new_init  = rewrite(old_init);
